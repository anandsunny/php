<?php
/*
Modification and Improvement  by Pankaj Boarh
1   @function  rawquery() 
    @param  query  query2
    @ reason raw query support for table prefix
2   @function  joinWhere()
    @reason Join works fine with table prefix but, joinWhere didnot which is fixed here to support table prefix i joinWhere
...
class MysqliDb
{
    ...
    public function connect()
    {
        ...
        if ($this->_mysqli->connect_error) {
            throw new Exception(die('<b>Oops! Database Connection Error, Please try again later</b><br>Note: If the problem persist, contact Strap Engine Team.'));//throw new Exception('Connect Error ' . $this->_mysqli->connect_errno . ': ' . $this->_mysqli->connect_error, $this->_mysqli->connect_errno);
        }
     ...
     */
    public function rawQuery($query,$query1, $bindParams = null)
    {
        $params = array(''); // Create the empty 0 index
        if(!empty($query1))
        $this->_query = $query.self::$prefix.$query1;
     ...
     */
    public function rawQueryOne($query,$query1, $bindParams = null)
    {
        $res = $this->rawQuery($query,$query1, $bindParams);
     ...
     */
    public function rawQueryValue($query,$query1, $bindParams = null)
    {
        $res = $this->rawQuery($query,$query1, $bindParams);
        if (!$res) {
            return null;
    ...
    {   
        if (!is_object($whereJoin)) {/*added by pankaj borah*/
            $whereJoin = self::$prefix . $whereJoin;
        }
